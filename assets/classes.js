class Particle{
	...
	drawSelf(){
		coDrawImage("heart", -1, this.x, this.y, 1, 0, 100-this.alpha, 1.5);
	}
}
...
class Bullet{
	...
	update(){
		...
		if(this.doParticleEffects){
			new_particle(this.x+randomize(-1, 1), this.y+randomize(-1, 1));
		}
	}
	drawSelf(debug){
		ctx.save();
...
class Arrow{
	...
	update(){
		...
		if(this.doParticleEffects){
			new_particle(this.x+randomize(-3, 3), this.y+randomize(-3, 3));
		}
	}
	drawSelf(debug){
...
class Skull{
	constructor(x, y, func, team, health, value){
		...
		this.doHealthRegeneration=(team==1);
		this.lastHealthGenerated=0;
	}
	drawSelf(){
		...
		if(!this.dying && (this.health_bar_show>0)){
			...
			ctx.moveTo(SCALE*(this.x-13.5+((this.health-this.lastHealthGenerated)/this.max_health)*27),SCALE*(this.y-60));
			ctx.lineTo(SCALE*(this.x-13.5+(this.health/this.max_health)*27),SCALE*(this.y-60));
			ctx.stroke();
...
function new_particle(x_=0, y_=400){
	np=new Particle(x_, y_);
	particles.push(np);
}
